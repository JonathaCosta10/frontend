...
export class Rules {
  ...
  async post(params: RequestParams): Promise<ApiResponse> {
    console.log("� Rules.post() START");
    
    const apiService = await this.getApiService();
    console.log("📝 Got apiService:", !!apiService);
    
    if (!apiService || !apiService.makeRequest) {
      console.error("❌ No apiService or makeRequest method");
      return { success: false, status: 500, message: "Service not available" };
    }

    try {
      console.log("� Calling apiService.makeRequest...");
      const result = await apiService.makeRequest("POST", params);
      console.log("� makeRequest result:", result);
      return result;
    } catch (error) {
      console.error("❌ Rules.post() error:", error);
      return { success: false, status: 500, message: "Request failed" };
    }
  }
  ...
  private async makeRequest(
    ...
    try {
      console.log("🌐 makeRequest:", method, endpoint);
...
export const login = async (
  ...
  console.log("🔐 LOGIN FUNCTION CALLED:", username, chave);

  try {
    console.log("🔐 Chamando rulesInstance.post...");
